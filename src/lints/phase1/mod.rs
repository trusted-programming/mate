...
impl Visitor<'_> for Validator {
    fn visit_expr(&mut self, ex: &Expr) {
        match &ex.kind {
            ExprKind::Try(_) 
                | ExprKind::Ret(_) 
                | ExprKind::Break(_, _) => self.has_early_ret = true,
            ExprKind::Await(e, _) => {
                self.is_async = true;
                self.visit_expr(&e)
            }
...
impl EarlyLintPass for ForEach {
    fn check_expr(&mut self, cx: &EarlyContext<'_>, expr: &Expr) {
        ...
        if let ExprKind::ForLoop(pat, iter, block, _) = &expr.kind {
            ...
            let mut cft = Validator { has_early_ret: false, is_async: false };
            cft.visit_block(&block);
            cft.visit_expr(&iter);
            if cft.has_early_ret || cft.is_async { return }
            ...
            // may apply.
            let suggestion = format!("({}).into_iter().for_each(|{}| {});", iter_snip, pat_snip, block_snip);
